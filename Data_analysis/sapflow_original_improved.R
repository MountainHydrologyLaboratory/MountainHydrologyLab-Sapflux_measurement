...
setwd("~/Desktop/Data_analysis/hillsideplot/") #set working directory
rm(list=ls())     #clear workspace
inmat<- as.matrix(read.csv("hillside3.csv", header=FALSE)[5:54024,6:26]) #reads datafile
n<-nrow(inmat) #read the sample size
trees<-ncol(inmat)/3 #number of trees
## Declare Datalogger Variables

ms<-148 #number of measurements per pulse
sec<-2 # seconds per interval
isec<-2 # seconds to calculate right time (before second after first reading)

#### Declare other variables
pul <-n/ms
ndays<-pul/48
stvar<- matrix(data=-9999,nrow=pul,ncol=trees) #should be NA to save space
temp<- matrix(data=-9999,nrow=pul,ncol=trees) #tempertature at the upstream/bottom probe for heat dissipation correction
w<-1.2 #wound width (mm) of the borehole: the needles are 1.14-1.19. 
d<-2.5 #determine the spacing between the heater and the middle of the two sensors
vwd<-0.5 #volume fraction of wood
vwt<-0.3 #volume fraction of water
error<- -9999
...
vh<-matrix(data = -9999,nrow=pul,ncol=trees)
...
vc<-matrix(data=-9999,nrow=pul,ncol=trees) #initialize vector to store corrected vh for wounding effects (vc)
...
for (t in 1:trees){ #tree loop
  ...
  for(p in 1:pul){#pulse cycle 
    ...
    } else if(vh[p,t]>0) { #conditional for vh in relation to 3cms/h (<) this condition only true whtn time=>300sec
...
vl<-matrix(data=-9999,nrow = pul, ncol = trees)
for (t in 1:trees){
  for (p in 1:pul){
    if (vh[p,t]>0){
...
write.csv(vl,"hillside3_results.csv")
